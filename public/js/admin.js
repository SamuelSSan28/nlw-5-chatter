    ...
    socket.emit("admin_list_messages_by_user", params, (messages) => {
        ...
        messages.forEach(message => {
            ...
            if (message.admin_id === null) {
                ...
                createDiv.innerHTML += `<span class="user_email">${connection.user.email}</span>`
                createDiv.innerHTML += `<p>${message.text}</p>`
                createDiv.innerHTML += `<span class="time-left">${dayjs(message.created_at).format("DD/MM/YYYY HH:mm:ss")}</span>`

            } else {
                createDiv.className = "admin_message_admin"
                createDiv.innerHTML = `<p>${message.text}</p>`
                createDiv.innerHTML += `<span class="time-right">${dayjs(message.created_at).format("DD/MM/YYYY HH:mm:ss")}</span>`
...
function sendMessage(id) {
    ...
    const createDiv = document.createElement("div")
    createDiv.className = "admin_message_admin"
    createDiv.innerHTML = `<p> ${params.text}</p>`
